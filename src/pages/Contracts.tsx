    ...
    <div className="p-6 max-w-screen-xl mx-auto">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{contracts.length} contracts · {contracts.filter(c => c.status === "active").length} active</p>
        </div>
        <button
      ...
      <div className="bg-white border border-[var(--border)] rounded-lg p-4 mb-4 flex items-center gap-3 flex-wrap">
        ...
        <select
          ...
          className="px-3 py-2 text-sm border border-[var(--border)] rounded text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-[var(--accent)]"
        >
        ...
        {(search || filterType !== "all" || filterRisk !== "all") && (
          <button
            onClick={() => { setSearch(""); setFilterType("all"); setFilterRisk("all"); }}
            className="px-3 py-2 text-sm text-slate-500 hover:text-slate-800 border border-[var(--border)] rounded hover:bg-slate-50 transition-colors"
          >
            Clear Filters
          </button>
        )}
      ...
      <div className="bg-white border border-[var(--border)] rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          <thead className="border-b border-[var(--border)] bg-slate-50">
            <tr>
              {["CONTRACT", "VENDOR", "TYPE", "STATUS", "RISK", "RENEWAL", "OBLIGATIONS", "UPDATED", ""].map((col) => (
                <th key={col} className="text-left text-xs font-medium text-slate-400 px-4 py-3">{col}</th>
              ))}
            </tr>
          </thead>
          <tbody>
        ...
        {filtered.length > 0 && (
          <div className="px-4 py-3 border-t border-[var(--border)] flex items-center justify-between bg-slate-50">
            <p className="text-xs text-slate-400">Showing {filtered.length} of {contracts.length} contracts</p>
            <div className="flex items-center gap-1">
              ...
              <button className="px-2.5 py-1 text-xs border border-[var(--border)] rounded text-slate-500 hover:bg-white transition-colors">Next →</button>
            </div>
          </div>
        )}
      </div>
